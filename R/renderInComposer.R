#' Render a htmlwidget in Alteryx Composer
#'
...
#' @export
renderInComposer = function(x, libdir = NULL, nOutput = 3, debug = FALSE,
    selfcontained = TRUE){
  if (!inAlteryx() && !debug){
  ...
  if (!selfcontained){
    ...
    html_deps <- NULL
    js <- lapply(scripts, function(x){paste(readLines(x), collapse = '\n')})
    ...
    #
    # js3 <- do.call(function(...)paste(..., collapse = '\n'), js2)
    css <- lapply(styles, function(x){paste(readLines(x), collapse = '\n')})
